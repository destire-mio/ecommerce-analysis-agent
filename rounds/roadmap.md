...
### M5 · 场景 S4 图表 / S5 报告（窄）
- **S3 口径解释 + 口径治理**：**已定案** → `rounds/S3-口径治理-实现文档.md`
- **S4 图表 / S5 报告（窄）**：**已定案** → `rounds/M5-S4S5-实现文档.md`
  - S4：单独工具 `chart`（bar/line/pie），输出 ECharts option JSON 落盘，M6 渲染
  - S5 窄：报告落盘时自动附"口径说明"节（从 MDL，按真实取数指标）+ 对账加一层
- 报告/图表更深优化（嵌图/模板/追溯/多格式）→ todo §D，不急
- **预估**：S3 一天；S4+S5 一天

### M6 · 产品承载（第 4 轮）
- **交付**（见 `rounds/M6-实现文档.md`）：Flask + Vue3 + ECharts（本地）最小全栈；提问→结论+结构化"查看依据"；多轮；轻冒烟 + 发布门禁
- **硬约束**：不新增业务功能；Runtime 即 `agent_loop.run_agent`
- **预估**：一天~一天半

### M7 · 治理与运维（最小版）
- **交付**（见 `rounds/M7-实现文档.md`）：① 三类条件分离 + **租户注入**（shop_id 从 ref_sql 拿到引擎注入）② 审计 `runs/audit.jsonl` + 查询脚本 ③ 列级权限（hidden 列）
- **不做（后续）**：血缘、temporal/多版本、影响分析、成本预算、缓存
- **预估**：一天
