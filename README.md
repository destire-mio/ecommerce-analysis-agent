...
- 超时计数按数据库、来源表和查询返回结构分组：统计项、去重方式、分组维度或明细字段不同，使用不同计数；普通别名、CTE 字段映射、过滤值、排序和 LIMIT 的变化不重置相同结构的计数。成功查询不清空其他查询的失败记录，60 秒审批只授权指定 SQL 一次。
- 分组是运行时启发式，不是 SQL 语义等价证明；复杂改写可能被分到不同组，同结构的不同业务问题也可能共用计数。每条未批准 SQL 仍受 5 秒限制，任务仍受 40 轮限制。升级旧会话时保留历史超时；只有尚未执行、由旧表级分组误拦的查询会恢复为 5 秒执行，真实超时和延期审批继续等待用户。
